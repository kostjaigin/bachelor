...
''' sample positive and negative sets for training and testing '''
...
def links2subgraphs(A, train_pos, train_neg, test_pos, test_neg, h=1,
    ...
    if h == 'auto':
        # split train into val-train and val_test
        _, _, val_test_pos, val_test_neg = sample_neg(A, 0.1)
        val_A = A.copy()
        val_A[val_test_pos[0], val_test_pos[1]] = 0  # mask negative samples
        val_A[val_test_pos[1], val_test_pos[0]] = 0  # mask negative samples
    ...
    def helper(A, links, g_label):
        g_list = [] # graphs
        if no_parallel:
            # go through pairs of links ...
            for i, j in tqdm(zip(links[0], links[1])):
                # ... and extract the graph for each pair
...
def subgraph_extraction_labeling(ind, A, h=1, max_nodes_per_hop=None, node_information=None):
    ...
    # extract the h-hop ecnlosing subgraph around link 'ind'
    dist = 0
    ...
    for dist in range(1, h+1):
        ...
        # not interested in nodes we have already visited
        fringe = fringe - visited
        # update visited
        visited = visited.union(fringe)
        if max_nodes_per_hop is not None:
            # random sampling of allowed number of nodes in a fringe
            if max_nodes_per_hop < len(fringe):
                fringe = random.sample(fringe, max_nodes_per_hop)
        if len(fringe) == 0:
            break
        # update nodes in subgraph
...
def node_label(subgraph):
    # an implementaion of the proposed double-radius node labeling
    K = subgraph.shape[0]
    K = subgraph.shape[0]
